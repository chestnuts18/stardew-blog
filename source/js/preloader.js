/**
 * 开场动画 — DOM烟花 + GSAP逐字弹跳
 * preloader div 在 layout 模板里，此脚本在 head 中用 rAF 等到 DOM 就绪
 */
(function () {
  try {
    var seen = sessionStorage.getItem("pl");

    function waitForDom(cb) {
      var el = document.getElementById("kuri-preloader");
      if (el) { cb(el); return; }
      requestAnimationFrame(function () { waitForDom(cb); });
    }

    waitForDom(function (pl) {
      if (seen) {
        // 第二次访问：CSS 已隐藏，直接移除 DOM
        pl.remove();
        return;
      }
      // 首次访问：显示 preloader，开始动画
      pl.style.display = "";
